...
<!-- <form action="additem.php" method="_GET">
    <input type="submit" value="Add item">
</form> -->

<form action="addNewItem.php" method="_GET">
...
// }
	...
	if (empty($popular)) {
	    if ($query != NULL) { 
	        ...
	        if ($var != NULL){
	        $id = 0;    
	        $q = "SELECT COUNT(orders.id) AS Count, products.id, products.name, products.description, products.price, products.created 
					FROM products 
					LEFT JOIN orders ON products.id = orders.user_id  
	            	WHERE products.name LIKE '%$query%'
	            	OR products.description LIKE '%$query%' 
	           		OR products.price LIKE '%$query%'
	        		AND products.type LIKE '$var'
	        ...
	        $q = "SELECT COUNT(orders.id) AS Count, products.id, products.name, products.description, products.price, products.created 
	            ...
	            if (mysqli_affected_rows($mysql) > 0) { 
	                ...
	                do {
	                    ...
	                    $text .= '<tr><td>'.$row['Count'].'</td><td><a href = "product_info.php?id='.$row['id'].'">'.$row['name'].'</td><td>'.$row['description'].'</td><td>'.$row['price'].'$'.'</td><td>'.$row['created'].'</td><td>'.$row['updated'].'</td><td>'.$row['type'].'</td><tr>';
	                    

	                } while ($row = mysqli_fetch_assoc($result3)); 
	                echo "<table><tr><th>Count</th><th>Name</th><th>description</th><th>price</th><th>created</th></tr>";
	                echo $text;
	                echo "</table>";
?>
...
<?php
	        ...
	        if ($var != NULL) {
	        	$query = "SELECT COUNT(orders.id) AS Count, products.id, products.name, products.description, products.price, products.created 
					FROM products 
					LEFT JOIN orders ON products.id = orders.user_id  
	        		WHERE products.type LIKE '$var'
	            	GROUP BY products.id
	            	LIMIT $quantity OFFSET $list";
	            // $query = "SELECT * FROM products WHERE `type` = '$var' LIMIT $quantity OFFSET $list";
	            //$query = "SELECT * FROM products WHERE price NOT LIKE '321'";

	            $result3 = mysqli_query($mysql, $query) or die("Error " . mysqli_error($mysql)); 
	            ...
	            //     echo "<table><tr><th>Count</th><th>Name</th><th>description</th><th>prise</th><th>created</th></tr>";
		...
		if ($query != NULL) { 
	        ...
	        $q = "SELECT COUNT(orders.id) AS Count, products.name, products.description, products.price, products.created 
					FROM products 
					LEFT JOIN orders ON products.id = orders.user_id  
	            	WHERE products.name LIKE '%$query%'
	            	OR products.description LIKE '%$query%' 
	        ...
	        if ($var != NULL) {
	            ...
	            $result3 = mysqli_query($mysql, $query) or die("Error " . mysqli_error($mysql)); 
	                ...
	                do {
	                    ...
	                    $text .= '<tr><td>'.$row['Count'].'</td><td>'.$row['name'].'</td><td>'.$row['description'].'</td><td>'.$row['price'].'</td><td>'.$row['created'].'</td><td>'.$row['updated'].'</td><td>'.$row['type'].'</td><tr>';
	        	...
	        	$query = "SELECT COUNT(orders.id) AS Count, products.name, products.description, products.price, products.created 
					FROM products
					LEFT JOIN orders ON products.id = orders.user_id
					GROUP BY products.id
					HAVING COUNT(orders.id) > 1";
